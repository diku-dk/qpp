...
import QPP hiding (fixpoint,simplifyPass)
...
--   dense matrix. Under QPP.Semantics.MPS this goes via the Convertible OpT CMat instance, which
...
{-|
   2. Evaluate the QFT program on 1, 2, 4, 10 qubits using the matrix semantics from QPP.Semantics.Matrix. Verify that the resulting matrices are unitary. What would happen if you tried this with 20 qubits? Use 'cabal repl ProgramInversion' to explore this interactively.
   -}
