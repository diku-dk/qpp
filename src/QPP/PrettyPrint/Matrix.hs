module QPP.PrettyPrint.Matrix where

import QPP.Syntax(RealT,ComplexT)
import QPP.Semantics(SparseMat(..),Convertible(..))
import QPP.Semantics.Matrix(CMat,RMat)
import QPP.Util(toBits', integerlog2)
...
showCMat :: Convertible t CMat => t -> String
showCMat mc' =
  let mc = to mc' :: CMat
      mr = cmap realPart mc
      mi = cmap imagPart mc
...
printM :: (Convertible t CMat) => t -> IO()
