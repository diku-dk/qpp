...
--   An identity is only dropped from a composition when its qubit count matches the other operand, so an
--   ill-formed term such as @Id 2 ∘ X@ is left for the backend to reject rather than silently "fixed".
cleanOnes :: QOp -> QOp
cleanOnes op = case op of
    ...
    Compose (Id n) b | n == op_qubits b -> cleanOnes b
    Compose a (Id n) | n == op_qubits a -> cleanOnes a
...
cleanAdjoints op = case op of
    ...
    Adjoint (Permute ks)      -> Permute (permInvert ks)
    --(AB)^-1 = B^-1 A^-1
...
doComp :: QOp -> QOp -> ([QOp], QOp)
doComp a b = case (a,b) of
    (Id n, x) | n == op_qubits x -> ([],x)
    (x, Id n) | n == op_qubits x -> ([],x)
    (X, X) -> ([],I)
    (Y, Y) -> ([],I)
    (Z, Z) -> ([],I)
    (H, H) -> ([],I)
...
pushComposes op = let push = pushComposes in case op of
    -- Push compositions down over tensors. Only valid when the two splits line up,
    -- e.g. (X ⊗ Id 2) ∘ (Id 2 ⊗ X) must not become (X ∘ Id 2) ⊗ (Id 2 ∘ X).
    Compose (Tensor a b) (Tensor c d)
      | op_qubits a == op_qubits c, op_qubits b == op_qubits d
      -> (push a ∘ push c) ⊗ (push b ∘ push d)
