...
module QPP.Semantics.Statevector
 {-| ( StateT
  , OpT(..)
  , apply
  , evalOp      -- :: QOp -> OpT
...
import Data.Bits(xor,testBit, setBit, clearBit)
--import qualified Torch as T
--import qualified Torch.Functional as F
--import qualified Torch.Tensor as TT
import Data.Massiv.Array
  ( Array
  , Ix2((:.))                    -- constructor imports (this is the key fix)
  , Sz(..)
  , D, U
  )
...
-- | Backend API
-- | This backend computes in the delayed representation `WorkT` and stores
--   states in the manifest, unboxed `StateT`; `HasWork` (from "QPP.Semantics")
--   is the conversion pair between the two.
instance HasWork WorkT where
  type Work WorkT = WorkT
  toWork   = id
  fromWork = id

instance HasWork StateT where
  type Work StateT = WorkT
  toWork   = A.delay
...
-- evalOp itself checks the op/state arity match, so the shared interpreter
...
measureProjection' :: (HasWork t, Work t ~ WorkT)
                               => Int  -- Arity of operator in qubits
                               -> Int  -- Batch dimension
                               -> Int  -- Qubit index to measure
...
opToDenseMat (OpT n f) =
  let columns = [ to (f (ket (toBits' n j))) :: CMat | j <- [0 .. pow2 n - 1] ]
...
-- `dagger` (structural adjoint) is shared across backends; it lives in
-- QPP.Syntax.
